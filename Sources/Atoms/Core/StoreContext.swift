...
internal struct StoreContext {
    ...
    func scoped(
        ...
        //sx.invalidateAllChildren()
        //sx.invalidateAllOverrides()
        sx.resetAllDanglingOverrides()
...
private extension StoreContext {
    ...
    private func invalidateAllChildren() {
        let store = getStore()
        
        for children in store.graph.children.values {
    ...
    public func invalidateAllOverrides() {
        for overrideValue in overrides.values {
            let atom = overrideValue.getAtom()
            invalidate(atom: atom)
        }
    }
    
    public func resetAllDanglingOverrides() {
        let store = getStore()
    ...
    public func resetDanglingOverride<Node: Atom>(atom: Node) {
        let override = lookupOverride(of: atom)
        let key = AtomKey(atom, overrideScopeKey: override?.scopeKey)
    ...
    public func invalidate<Node: Atom>(atom: Node) {
        let override = lookupOverride(of: atom)
        let key = AtomKey(atom, overrideScopeKey: override?.scopeKey)
